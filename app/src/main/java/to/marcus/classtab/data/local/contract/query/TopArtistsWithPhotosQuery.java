 ...
 * Returns Top artists containing photos (by access time)
 */
public class TopArtistsWithPhotosQuery implements SQLQueryStatement {
    @Override
    public String sqlQuery(Object params) {
        return String.format("SELECT A.*, P.* FROM %1s A INNER JOIN %2s P ON A."
                        ...
                        +" WHERE A.%3s = P.%4s ORDER BY "+ClassTabDB.ArtistTable.COLUMN_ID+" LIMIT 9;"
